...
for (i in 1:(ncol(fit)-1)){
  for (j in (i+1):ncol(fit)){
    ...
    #df <- y[[1]][rownames(y[[1]]) %in% rownames(lrt), c(which(sapply(colnames(y[[1]]), function(x) strsplit(x, "[.]")[[1]][1]) %in% (colnames(fit)[i])), which(sapply(colnames(y[[1]]), function(x) strsplit(x, "[.]")[[1]][1]) %in% (colnames(fit)[j])))]
    ...
    listDEG[[paste0(colnames(fit)[i], colnames(fit)[j])]] <- lrt
    #lrtLength <- nrow(lrt)
...
saveRDS(listDEG, file=paste0(thisPath, "/DEGPairs/listDEG.Rds"))
```
