...
for (i in 1:(ncol(fit)-1)){
  for (j in (i+1):ncol(fit)){
    paste0(unique(Group)[i], "_", unique(Group)[j])
    length(which(bees_metrics[[paste0(unique(Group)[i], "_", unique(Group)[j])]]$FDR <0.05))
  }
}

save(bees_metrics, file = "../../data/bees_metrics_48.rda")
