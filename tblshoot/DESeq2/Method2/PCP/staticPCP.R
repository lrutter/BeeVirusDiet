...
outDir = "/Users/lindz/BeeVirusDiet/tblshoot/DESeq2/Method2/PCP"
...
#
# countdata <- dat[, order(unlist(lapply(names(dat), function (x) substring(unlist(strsplit(x, "[.]"))[1],2,2))))]
#
...
dds <- readRDS("/Users/lindz/BeeVirusDiet/tblshoot/DESeq2/Method2/beeDataDDSRLD.rds")[[1]]
rld <- readRDS("/Users/lindz/BeeVirusDiet/tblshoot/DESeq2/Method2/beeDataDDSRLD.rds")[[2]]
...
sampleIndex <- which(sapply(colnames(assay(rld)), function(x) substr(unlist(strsplit(x,"[.]")), 1, 1)[1]) %in% c(group1, group2))
...
p <- ggplot(boxDat, aes(x = Sample, y = Count)) + geom_boxplot() + geom_line(data=pcpDat2, aes(x = Sample, y = Count, group = ID), size = 0.2, alpha = 1, color = "red") + theme(axis.text.x=element_text(angle=90, hjust=1, size = 12))
