...
p <- ggplot(boxDat, aes(x = Sample, y = Count)) + geom_boxplot() + geom_line(data=pcpDat2, aes(x = Sample, y = Count, group = ID), size = 0.3, alpha = 0.6, color = "red") + theme(axis.text.x=element_text(angle=90, hjust=1, size = 12))
