...
server <- shinyServer(function(input, output, session) {
  ...
  #   nVar = reactive(ncol(bindataSel()))
  #   colNms <- reactive(colnames(bindataSel()[, c(2:nVar())]))
  #   boxDat <- reactive(bindataSel()[, c(1:nVar())] %>% gather(key, val, -c(ID)))
